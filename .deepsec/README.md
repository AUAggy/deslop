...
Currently configured project: `vscode-dslop` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/vscode-dslop/SETUP.md` to fill in
   `data/vscode-dslop/INFO.md`.
...
`--project-id vscode-dslop` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/vscode-dslop/`.
